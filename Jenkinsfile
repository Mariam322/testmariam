pipeline {
                    ...
                    sshagent(['deploy-key']) {
                        sh """
                            ...
                            ssh -o StrictHostKeyChecking=no ${DEPLOY_HOST} \
                              "echo \$P | docker login -u \$U --password-stdin && \
                               cd ${DEPLOY_DIR} && \
                               IMAGE_TAG=${TAG} docker compose up -d --pull always testmariam && \
                               sudo /usr/local/bin/harice-nginx-install nginx-site.conf testmariam"
                        """
                    }
                }
            }
    ...
    post {
        failure { echo 'Echec pipeline testmariam' }
        success { echo 'Pipeline testmariam OK' }
    }
}
